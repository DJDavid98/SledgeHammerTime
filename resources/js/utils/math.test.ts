...
describe('Math Utils', () => {
    ...
    describe('integerInRangeByAngle', () => {
        ...
        it('should return correct value for minute clock positions', () => {
            const maxMinutes = 60;
            const anglesPerMinutes = 360 / maxMinutes;
            for (let minutes = 0; minutes < maxMinutes; minutes++) {
                expect(integerInRangeByAngle(0, maxMinutes, anglesPerMinutes * minutes), `correct value for minute ${minutes}`).to.eql(minutes);
            }
        });
        it('should return correct value when closer to next value', () => {
            const maxHours = 12;
            const anglesPerHour = 360 / maxHours;
            expect(integerInRangeByAngle(0, maxHours, anglesPerHour * 11.5)).to.eql(0);
        });
        it('should return correct value when closer to previous value', () => {
            const maxHours = 12;
            const anglesPerHour = 360 / maxHours;
            expect(integerInRangeByAngle(0, maxHours, anglesPerHour * 11.49)).to.eql(11);
        });
    });
});
