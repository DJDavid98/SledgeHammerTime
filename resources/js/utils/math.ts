...
/*
 * The coordinate system in browsers has the positive values towards the bottom right
 *
 * -x-y   0°  +x-y
 *        |
 * 270° - 0 -- 90°
 ...
 * -x+y  180° +x+y
 */
export const getQuadrantBaseAngle = (quadrant: PointQuadrant): number => {
	switch (quadrant) {
...
export const integerInRangeByAngle = (minValue: number, maxValue: number, angle: number) =>
	Math.min(Math.round(minValue + (angle / 360) * (maxValue - minValue)), maxValue - 1);
